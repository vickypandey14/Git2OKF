...
pub async fn clone_repository(url: &str, target_path: &Path, depth: Option<u32>) -> Result<Repository, Git2OkfError> {
    ...
    match timeout(Duration::from_secs(60), clone_task).await {
        ...
        Err(_) => Err(Git2OkfError::TimeoutError("Clone operation timed out after 60 seconds".to_string())),
    }
}
