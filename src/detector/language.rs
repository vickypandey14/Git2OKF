...
pub fn analyze_files_and_languages(path: &Path) -> Result<(Vec<LanguageStat>, FileStats), Git2OkfError> {
        ...
        .filter(|e| e.file_type().is_file())
    {
        ...
        if let Some(ext) = entry.path().extension().and_then(|e| e.to_str()) {
            ...
            match ext.as_str() {
                ...
                _ => {
                    // Check specific names for configs if extension didn't match
